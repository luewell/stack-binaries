...
| | |
|---|---|
| **Stack** | The CLI itself, for macOS arm64, Linux amd64 and arm64 and Windows amd64 and arm64, released as `stack-<version>`. |
...
| **BusyBox for Windows** | Ron Yorston's busybox-w32, which Stack runs the catalogue's shell scripts with on Windows, mirrored unchanged from frippery.org, its only publisher, so a Windows machine can run services when that site refuses. Published only when the bytes match the digests the catalogue pinned after the author's signed `SHA256SUM` verified; the Internet Archive's copy is taken when frippery.org refuses. GPL-2.0, with the release's source attached. Released as `busybox-w32-<release>`. |
...
A release is exactly five archives, `SHA256SUMS` and the `SOURCE_COMMIT` they
